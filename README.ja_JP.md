...
インデックス化を有効にしたときの実行例:  
(この例では、インデックス化をしていないときは50秒かかっていたものが、5秒に短縮されました。)  
![](images/example2.png)
...
インデックス内からの検索の例。1千万個以上のテキストファイル、9分以内。  
![](images/example4.png)
...
- [ ] パフォーマンス向上のためのDB構造の変更 (v2)
- [x] 検索対象の文書ファイルが数百万個になったときのための明示的なインデックス化コマンド (v2)
- [x] GC機能。インデックスDBから対応するファイルが存在しないインデックスデータを削除する (v2)
- [ ] 対応するファイルのタイムスタンプが数秒ずれていたときに、インデックスデータが古くなったとみなされないように修正する
