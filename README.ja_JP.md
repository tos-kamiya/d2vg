...
`--normalize-vector, -n`  
クエリとの類似性を計算するとき、文書の分散表現であるベクタを単位ベクトルへと正規化する。パラグラフの長さの差が大きい場合は、短いパラグラフが優先的に検索結果に表示されるようになります。
...
```sh
cd 文書ファイルのディレクトリ
d2vg --indexing -j <ワーカープロセスの数> <ファイル>...
```
...
```sh
cd 文書ファイルのディレクトリ
d2vg --cached -j <ワーカープロセスの数> <クエリとなるフレーズ>
```
